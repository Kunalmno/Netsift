    ...
    return (
        <Card className="border border-separate shadow-sm rounded-lg overflow-hidden hover:shadow-md dark:shadow-primary/30 group/card">
            <CardContent className="p-4 flex items-center justify-between h-[100px]">
                <div className="flex items-center jusitfy-end space-x-3">
                    <div
                        className={cn(
                    ...
                    <div>
                        ...
                        <TooltipWrapper content={workflow.description}>
                            <Link 
                                ...
                                className="flex itmes-center hover:underline">
                                    {workflow.name}
                            </Link>
                        </TooltipWrapper>
                            {isDraft && (
                                <span className="ml-2 px-2 py-2 text-xs font-medium bg-yellow-100 text-yellow-800 rounded-full">
                                    Draft
                                </span>
                            )}
    ...
    <div className="flex items-center gap-2">
        ...
        <TooltipWrapper content="credits consumption for full run">
            <div className="flex items-center gap-3">
                <Badge
                    variant={"outline"}
                    className="spae-x-2 text-muted-foregorund rounded-sm"
                >
...
function LastRunDetails({workflow}:{workflow: Workflow}){
    ...
    const nextSchedule = nextRunAt && format(nextRunAt, "yyy-MM-dd HH:mm");
    const nextScheduleUTC  = nextRunAt && formatInTimeZone(nextRunAt, "UTC", "HH:mm");
    return (
    <div className="bg-primary/5 px-4 py-1 flex justify-between items-center text-muted-foreground">
        <div className="flex items-center text-sm gap-2">
            {lastRunAt && (
                <Link href={`/workflow/runs/${workflow.id}/${lastRunId}`} 
